...
export default function VirtualMall() {
  ...
  // ============================================
  // MULTIPLAYER SOCKET - SEPARATE FROM VIDEO CALL
  // ============================================
  // This socket is ONLY for player position synchronization in the virtual world
  // It is COMPLETELY SEPARATE from the video call socket used in TVScreen.jsx
  // 
  // Video Call Socket: http://localhost:3000/meeting (in TVScreen.jsx)
  // Player Socket:     http://localhost:3000/player (this one - VirtualMall.jsx)
  // ============================================
  
  // Multiplayer state - track remote players
  const [remotePlayers, setRemotePlayers] = useState({})
  const playerSocketRef = useRef(null) // Player socket - separate from video call socket
  const lastPositionRef = useRef([0, 0, 0])
  const lastRotationRef = useRef(0)
  
  // Initialize Socket.IO connection for MULTIPLAYER (player positions only)
  useEffect(() => {
    ...
    const playerSocket = io('http://localhost:3000/player')
    ...
    playerSocket.on('current-players', (players) => {
      ...
      if (players && typeof players === 'object') {
        ...
        const playersObj = Array.isArray(players) 
          ? players.reduce((acc, p) => ({ ...acc, [p.id]: p }), {})
          : players
        setRemotePlayers(playersObj)
        console.log(`🔵 Rendering ${Object.keys(playersObj).length} remote players`)
      } else {
        setRemotePlayers({})
      }
    })
    ...
    playerSocket.on('new-player', (playerData) => {
      ...
      if (playerData && playerData.id) {
        setRemotePlayers(prev => {
          ...
          console.log(`🔵 Total remote players now: ${Object.keys(updated).length}`)
          return updated
        })
      }
    })
    ...
    playerSocket.on('player-moved', (data) => {
      if (!data || !data.id) return
      
      setRemotePlayers(prev => {
        if (data.id === playerSocket.id) {
        ...
        const updated = {
          ...
          [data.id]: {
            ...prev[data.id],
            id: data.id,
            position: data.position || prev[data.id]?.position || [0, 0, 0],
            rotation: data.rotation || prev[data.id]?.rotation || [0, 0, 0]
          }
        }
        return updated
      })
    ...
    playerSocket.on('player-left', (id) => {
  ...
  // Send position updates to MULTIPLAYER server (not video call server)
  useFrame(() => {
    if (!playerSocketRef.current) return
    
    const currentPosition = usePlayerStore.getState().position
    const currentRotation = usePlayerStore.getState().rotation
    
    // Only emit if position or rotation changed significantly (to save bandwidth)
    const posChanged = Math.abs(currentPosition[0] - lastPositionRef.current[0]) > 0.1 ||
                       Math.abs(currentPosition[1] - lastPositionRef.current[1]) > 0.1 ||
                       Math.abs(currentPosition[2] - lastPositionRef.current[2]) > 0.1
    
    const rotChanged = Math.abs(currentRotation - lastRotationRef.current) > 0.1
    
    if (posChanged || rotChanged) {
      playerSocketRef.current.emit('move', {
  ...
  return (
    <>
      ...
      {/* Other Players - Ghost models for remote players */}
      {Object.values(remotePlayers)
        .filter((player) => {
          // Don't render our own player
          if (playerSocketRef.current && player.id === playerSocketRef.current.id) {
            return false
          }
        ...
        .map((player) => {
          // Ensure position is valid array - Y should be 0 for ground level
          const pos = Array.isArray(player.position) && player.position.length >= 3
            ? [player.position[0], 0, player.position[2]] // Force Y to 0 (ground level)
            : [0, 0, 0]
          
          // Ensure rotation is valid
          const rot = Array.isArray(player.rotation) && player.rotation.length >= 3
            ? player.rotation
            : (typeof player.rotation === 'number' ? [0, player.rotation, 0] : [0, 0, 0])
          
          return (
            <Suspense key={player.id} fallback={
              <group position={pos}>
                <mesh castShadow>
      ...
      {/* Debug: Show count of remote players */}
